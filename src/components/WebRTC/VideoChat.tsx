...
const VideoChat: React.FC<VideoChatProps> = ({ roomId }) => {
	...
	useEffect(() => {
		...
		const iceCandidateListenerCleanup = listenForIceCandidate(
			roomId,
	...
	useEffect(() => {
		const remoteParticipantRef = ref(db, `rooms/${roomId}/participants/`);
				...
				{showAddUser && (
					<div className="flex flex-col justify-center space-y-4 absolute top-24 bg-gray-200 dark:bg-gray-700 rounded-lg p-8 items-center space-x-2">
						...
						<button
							...
							className="bg-blue-600 hover:bg-blue-500 dark:bg-purple-600 hover:dark:bg-purple-500 text-white p-2 rounded"
						>
							Add User
						</button>
						<button
							onClick={hideAddUserForm}
							className="text-gray-500 hover:text-blue-500 dark:hover:text-purple-500"
						>
							Cancel
						</button>
					</div>
