  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (completeMessage !== '' || selectedGif) {
        // If there is text or a selected GIF, create and send the message
  ...
  return (
    <div className="flex flex-grow h-32 border-t border-l bg-gray-100 text-gray-700 dark:bg-gray-900 dark:border-gray-600 items-center space-x-2 w-full">
      <div className="flex flex-grow items-center w-full ml-5 mr-2 space-x-2">
        ...
        <HiOutlineGif onClick={() => setShowGif(true)} size={30} className="cursor-pointer stroke-blue-500 hover:stroke-blue-500/90 dark:stroke-purple-600/90 hover:dark:stroke-purple-600" />
      ...
      {showGif && (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-gray-100 dark:bg-gray-800 p-5 rounded-md">
            <GiphySearch API_SEARCH={API_SEARCH} API_KEY={API_KEY} onGifClick={handleGifClick} />
